...
__device__ static int get_sqlite_pointer(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Jim_CmdInfo cmdInfo;
	if (!Jim_GetCommandInfo(interp, Jim_GetString(args[1]), &cmdInfo))
	{
		Jim_AppendResult(interp, "command not found: ", args[1], nullptr);
		return JIM_ERROR;
	}
...
__device__ int getDbPointer(Jim_Interp *interp, const char *a, Context **ctx)
{
	struct TclContext *p;
	Jim_CmdInfo cmdInfo;
	if (Jim_GetCommandInfo(interp, a, &cmdInfo))
	{
...
__device__ static int test_io_trace(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
#if !defined(OMIT_TRACE) && defined(ENABLE_IOTRACE)
	if (argc != 2)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " FILENAME\"", nullptr);
		return JIM_ERROR;
	}
	if (_iotrace_file)
	{
	...
	if (args[1][0])
	{
		if (!_strcmp(args[1], "stdout")) _iotrace_file = _stdout;
		else if (_strcmp(args[1], "stderr")) _iotrace_file = _stderr;
		else _iotrace_file = _fopen(args[1], "w");
...
__device__ static int test_exec_printf(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	if (argc != 4)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " DB FORMAT STRING", nullptr);
		return JIM_ERROR;
	}
	Context *ctx;
...
__device__ static int test_exec_hex(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	if (argc != 3)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " DB HEX", nullptr);
		return JIM_ERROR;
	}
	Context *ctx;
	if (getDbPointer(interp, args[1], &ctx)) return JIM_ERROR;
...
__device__ static int db_enter(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	Context *ctx;
	if (argc != 2)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " DB", nullptr);
		return JIM_ERROR;
	}
	if (getDbPointer(interp, args[1], &ctx)) return JIM_ERROR;
...
__device__ static int db_leave(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	Context *ctx;
	if (argc != 2)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " DB", 0);
		return JIM_ERROR;
	}
	if (getDbPointer(interp, args[1], &ctx)) return JIM_ERROR;
...
__device__ static int test_exec(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	if (argc != 3)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " DB SQL", nullptr);
		return JIM_ERROR;
	}
	Context *ctx;
	if (getDbPointer(interp, args[1], &ctx)) return JIM_ERROR;
...
__device__ static int test_exec_nr(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	char *err = nullptr;
	if (argc != 3)
	{
...
// Return the result.
__device__ static int test_mprintf_z(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	char *result = nullptr;
...
__device__ static int test_mprintf_n(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	int n = 0;
...
__device__ static int test_snprintf_int(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	int n = _atoi(args[1]);
...
// one %s or %q.  STRING is the value inserted into %s or %q.
__device__ static int test_get_table_printf(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	int resCount = -1;
	if (argc == 5)
	...
	if (argc != 4 && argc != 5)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " DB FORMAT STRING ?COUNT?", nullptr);
		return JIM_ERROR;
	}
	Context *ctx;
	if (getDbPointer(interp, args[1], &ctx)) return JIM_ERROR;
...
__device__ static int test_last_rowid(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	if (argc != 2)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " DB\"", nullptr);
...
//
// Set the codec key.
__device__ static int test_key(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
#ifdef HAS_CODEC
	if (argc != 3)
	{
...
__device__ static int test_rekey(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
#ifdef HAS_CODEC
	if (argc != 3)
	{
...
__device__ static int sqlite_test_close(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	if (argc != 2)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " FILENAME\"", nullptr);
	...
	Jim_SetResult(interp, (char *)t1ErrorName(rc), JIM_STATIC);
	return JIM_OK;
}
...
__device__ static int test_create_function(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	if (argc != 2)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " DB\"", nullptr);
...
__device__ static int test_create_aggregate(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	if (argc != 2)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " FILENAME\"", nullptr);
...
__device__ static int test_printf(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	if (argc != 2)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " TEXT\"", nullptr);
		return JIM_ERROR;
	}
	_printf("%s\n", args[1]);
	return JIM_OK;
...
__device__ static int sqlite3_mprintf_int(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	if (argc != 5)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " FORMAT INT INT INT\"", nullptr);
		return JIM_ERROR;
	}
	int a[3];
	for (int i = 2; i < 5; i++)
...
__device__ static int sqlite3_mprintf_int64(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	if (argc != 5)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " FORMAT INT INT INT\"", nullptr);
...
__device__ static int sqlite3_mprintf_long(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	if (argc != 5)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " FORMAT INT INT INT\"", nullptr);
...
__device__ static int sqlite3_mprintf_str(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	if (argc < 4 || argc > 5)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " FORMAT INT INT ?STRING?\"", nullptr);
		return JIM_ERROR;
	}
	int a[3];
	for (int i = 2; i < 4; i++)
...
//
// Call mprintf with two integer arguments and one string argument
__device__ static int sqlite3_snprintf_str(ClientData notUsed, Jim_Interp *interp, int argc, const char *args[])
{
	if (argc < 5 || argc > 6)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " INT FORMAT INT INT ?STRING?\"", nullptr);
		return JIM_ERROR;
	}
	int n;
	...
	if (n < 0)
	{
		Jim_AppendResult(interp, "N must be non-negative", 0);
		return JIM_ERROR;
	}
	int a[3];
	for (int i = 3; i < 5; i++)
...
__device__ static int sqlite3_mprintf_double(ClientData notUsed, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 5)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " FORMAT INT INT DOUBLE\"", nullptr);
		return JIM_ERROR;
	}
	int a[3];
	for (int i = 2; i < 4; i++)
...
__device__ static int sqlite3_mprintf_scaled(ClientData notUsed, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 4)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " FORMAT DOUBLE DOUBLE\"", nullptr);
		return JIM_ERROR;
	}
	double r[2];
	for (int i = 2; i < 4; i++)
...
__device__ static int sqlite3_mprintf_stronly(ClientData notUsed, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 3)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " FORMAT STRING\"", 0);
		return JIM_ERROR;
	}
	char *z = _mprintf(args[1], args[2]);
	Jim_AppendResult(interp, z, 0);
	_free(z);
	return JIM_OK;
}
...
__device__ static int sqlite3_mprintf_hexdouble(ClientData notUsed, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 3)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " FORMAT STRING\"", nullptr);
		return JIM_ERROR;
	}
	unsigned int x1, x2;
...
__device__ static int test_enable_shared(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	if (argc == 2)
	{
		...
		if (rc != RC_OK)
		{
			Jim_SetResult(interp, (char *)Main::ErrStr(rc), JIM_STATIC);
			return JIM_ERROR;
		}
	}
	Jim_SetResult(interp, ret ? "1" : "0", nullptr);
	return JIM_OK;
}
#endif
...
{
	Jim_SetResult(interp, CORE_VERSION, nullptr);
	return JIM_OK;
}
...
__device__ static int test_table_column_metadata(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	if (rc != RC_OK)
	{
		Jim_AppendResult(interp, Main::ErrMsg(ctx), nullptr);
		return JIM_ERROR;
	}
	Jim_Obj *r = Jim_NewObj();
	Jim_ListObjAppendElement(nullptr, r, datatype);
	Jim_ListObjAppendElement(nullptr, r, collseq);
	Jim_ListObjAppendElement(nullptr, r, (int)notnull);
	Jim_ListObjAppendElement(nullptr, r, (int)primarykey);
	Jim_ListObjAppendElement(nullptr, r, (int)autoincrement);
...
__device__ static int blobHandleFromObj(Jim_Interp *interp, const char *arg, Blob **blob)
{
	int n;
	char *z = Jim_GetString(interp, arg, &n);
	if (n == 0)
		*blob = nullptr;
	else
	{
...
__device__ static int test_blob_read(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	if (rc == RC_OK)
		Jim_SetObjResult(interp, array_t<const void>(buf, bytes));
	else
...
__device__ static int test_blob_reopen(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	if (blobHandleFromObj(interp, args[1], &blob)) return JIM_ERROR;
	int64 rowid;
	if (Jim_GetWideInt(interp, args[2], &rowid)) return JIM_ERROR;
...
__device__ static void testCreateCollationDel(void *ctx)
{
	...
	int rc = Jim_Eval(p->Interp, (char *)p->Del, 0, nullptr);
	if (rc != JIM_OK)
		Jim_BackgroundError(p->Interp);
	Jim_DecrRefCount(p->Cmp);
	Jim_DecrRefCount(p->Del);
	_free((void *)p);
}
__device__ static int testCreateCollationCmp(void *ctx, int leftLength, const void *left, int rightLength, const void *right)
{
	TestCollationX *p = (TestCollationX *)ctx;
	char *script = Jim_DuplicateObj(p->Cmp);
	Jim_IncrRefCount(script);
	Jim_ListObjAppendElement(nullptr, script, Jim_NewStringObj((char *)left, leftLength));
	Jim_ListObjAppendElement(nullptr, script, Jim_NewStringObj((char *)right, rightLength));
	int res = 0;
	if (Jim_Eval(p->Interp, script, 0, nullptr) != JIM_OK || Jim_GetInt(p->Interp, p->Interp->result, &res) != JIM_OK)
		Jim_BackgroundError(p->Interp);
...
__device__ static void cf2Destroy(void *user)
{
	...
	{
		int rc = Jim_Eval(p->Interp, p->Destroy, 0, nullptr);
		if (rc != JIM_OK) Jim_BackgroundError(p->Interp);
	}
	if (p->Func) Jim_DecrRefCount(p->Func); 
	if (p->Step) Jim_DecrRefCount(p->Step); 
	if (p->Final) Jim_DecrRefCount(p->Final); 
	if (p->Destroy) Jim_DecrRefCount(p->Destroy); 
	_free(p);
}
...
__device__ static int test_create_function_v2(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	int enc;
	if (Jim_GetIndex(interp, args[4], (const void **)_encs, sizeof(_encs[0]), "encoding", 0, &enc)) return JIM_ERROR;
	enc = _encs[enc].Enc;
	...
	for (int i = 5; i < argc; i+=2)
	{
		...
		if (Jim_GetIndex(interp, args[i], switchName, "switch", 0, &switchId))
		{
			_free(p);
			return JIM_ERROR;
		}
	...
	if (p->Func) p->Func = Jim_DuplicateObj(p->Func); 
	if (p->Step) p->Step = Jim_DuplicateObj(p->Step); 
	if (p->Final) p->Final = Jim_DuplicateObj(p->Final); 
	if (p->Destroy) p->Destroy = Jim_DuplicateObj(p->Destroy); 
	...
	{
		Jim_ResetResult(interp);
		Jim_AppendResult(interp, sqlite3TestErrorName(rc), 0);
		return JIM_ERROR;
	}
	return JIM_OK;
}
...
__device__ static int test_load_extension(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Jim_CmdInfo cmdInfo;
	if (!Jim_GetCommandInfo(interp, dbName, &cmdInfo))
	{
	...
	if (rc != RC_OK)
	{
		Jim_SetResult(interp, (err ? err : ""), JIM_VOLATILE);
		rc = JIM_ERROR;
	}
	else
		rc = JIM_OK;
...
__device__ static int test_register_func(ClientData notUsed, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 3)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " DB FUNCTION-NAME", nullptr);
		return JIM_ERROR;
	}
	Context *ctx;
	if (getDbPointer(interp, args[1], &ctx)) return JIM_ERROR;
	...
	if (rc != 0)
	{
		Jim_AppendResult(interp, Main::ErrStr(rc), nullptr);
		return JIM_ERROR;
	}
	if (sqlite3TestErrCode(interp, ctx, rc)) return JIM_ERROR;
	return JIM_OK;
...
__device__ static int test_finalize(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 2)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " <STMT>", nullptr);
		return JIM_ERROR;
	}
	Vdbe *stmt;
	if (getStmtPointer(interp, args[1], &stmt)) return JIM_ERROR;
...
__device__ static int test_next_stmt(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 3)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " DB STMT", nullptr);
		return JIM_ERROR;
	}
	Context *ctx = nullptr;
	if (getDbPointer(interp, args[1], &ctx)) return JIM_ERROR;
	Vdbe *stmt;
	if (getStmtPointer(interp, args[2], &stmt)) return JIM_ERROR;
...
__device__ static int uses_stmt_journal(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Jim_SetObjResult(interp, (bool)((Vdbe *)stmt)->UsesStmtJournal);
	return JIM_OK;
}
...
__device__ static int test_expired(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	//	Vdbe *stmt;
	//	if (getStmtPointer(interp, args[1], &stmt)) return JIM_ERROR;
	//	Jim_SetObjResult(interp, Main::Expired(stmt)));
	//#endif
	return JIM_OK;
}
...
__device__ static int test_transfer_bind(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	if (argc != 3)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " FROM-STMT TO-STMT", nullptr);
		return JIM_ERROR;
	}
	Vdbe *stmt1, *stmt2;
	if (getStmtPointer(interp, args[1], &stmt1)) return JIM_ERROR;
	if (getStmtPointer(interp, args[2], &stmt2)) return JIM_ERROR;
	Jim_SetObjResult(interp, Vdbe::TransferBindings(stmt1, stmt2));
#endif
	return JIM_OK;
}
...
__device__ static int test_changes(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Context *ctx;
	if (getDbPointer(interp, args[1], &ctx)) return JIM_ERROR;
	Jim_SetObjResult(interp, Main::CtxChanges(ctx));
	return JIM_OK;
}
...
__device__ static int test_bind(ClientData notUsed, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 5)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " VM IDX VALUE (null|static|normal)\"", nullptr);
		return JIM_ERROR;
	}
	Vdbe *stmt;
	if (getStmtPointer(interp, args[1], &stmt)) return JIM_ERROR;
	...
	if (rc)
	{
		char buf[50];
		_sprintf(buf, "(%d) ", rc);
		Jim_AppendResult(interp, buf, Main::ErrStr(rc), nullptr);
		return JIM_ERROR;
	}
	return JIM_OK;
}
...
__device__ static int test_collate_func(void *ctx, int aLength, const void *a, int bLength, const void *b)
{
	...
	char *pX = Jim_NewStringObj("test_collate", -1);
	Jim_IncrRefCount(pX);
	switch (encin)
	{
	case TEXTENCODE_UTF8: Jim_ListObjAppendElement(interp, pX, "UTF-8"); break;
	case TEXTENCODE_UTF16LE: Jim_ListObjAppendElement(interp, pX, "UTF-16LE"); break;
	case TEXTENCODE_UTF16BE: Jim_ListObjAppendElement(interp, pX, "UTF-16BE"); break;
	...
	if (val)
	{
		...
		Jim_ListObjAppendElement(interp, pX, Jim_NewStringObj((char *)Vdbe::Value_Text(val), n));
	...
	_benignalloc_end();
	Jim_Eval(interp, pX, 0, nullptr);
	Jim_DecrRefCount(pX);
	int res;
	Jim_GetInt(interp, interp->result, &res);
	return res;
}
__device__ static int test_collate(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	return JIM_OK;
bad_args:
	Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " <DB> <utf8> <utf16le> <utf16be>", nullptr);
	return JIM_ERROR;
}
...
__device__ static void test_function_utf8(FuncContext *fctx, int argc, Mem **args)
{
	...
	char *pX = Jim_NewStringObj("test_function", -1);
	Jim_IncrRefCount(pX);
	Jim_ListObjAppendElement(interp, pX, "UTF-8");
	Jim_ListObjAppendElement(interp, pX, Jim_NewStringObj((char *)Vdbe::Value_Text(args[0]), -1));
	Jim_Eval(interp, pX, 0, nullptr);
	Jim_DecrRefCount(pX);
...
__device__ static void test_function_utf16le(FuncContext *fctx, int argc, Mem **args)
{
	...
	char *pX = Jim_NewStringObj("test_function", -1);
	Jim_IncrRefCount(pX);
	Jim_ListObjAppendElement(interp, pX, "UTF-16LE");
	Jim_ListObjAppendElement(interp, pX, Jim_NewStringObj((char*)Vdbe::Value_Text(args[0]), -1));
	Jim_Eval(interp, pX, 0, nullptr);
	Jim_DecrRefCount(pX);
	Mem *val = Vdbe::ValueNew(nullptr);
...
__device__ static void test_function_utf16be(FuncContext *fctx,  int argc, Mem **args)
{
	...
	char *pX = Jim_NewStringObj("test_function", -1);
	Jim_IncrRefCount(pX);
	Jim_ListObjAppendElement(interp, pX, "UTF-16BE");
	Jim_ListObjAppendElement(interp, pX, Jim_NewStringObj((char *)Vdbe::Value_Text(args[0]), -1));
	Jim_Eval(interp, pX, 0, nullptr);
	Jim_DecrRefCount(pX);
	Mem *val = Vdbe::ValueNew(nullptr);
...
__device__ static int test_errstr(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Jim_SetResult(interp, (char *)Main::ErrStr((RC)i), nullptr);
	return JIM_OK;
}
...
__device__ static int test_bind_int(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 4)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " STMT N VALUE", nullptr);
		return JIM_ERROR;
	}
	Vdbe *stmt;
	if (getStmtPointer(interp, args[1], &stmt)) return JIM_ERROR;
...
__device__ static int test_bind_int64(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	if (Jim_GetInt(interp, args[2], &idx)) return JIM_ERROR;
	int64 value;
	if (Jim_GetWideInt(interp, args[3], &value)) return JIM_ERROR;
...
__device__ static int test_bind_null(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 3)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " STMT N", nullptr);
		return JIM_ERROR;
	}
	Vdbe *stmt;
	if (getStmtPointer(interp, args[1], &stmt)) return JIM_ERROR;
...
__device__ static int test_bind_text(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 5)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " STMT N VALUE BYTES", nullptr);
		return JIM_ERROR;
	}
	Vdbe *stmt;
	if (getStmtPointer(interp, args[1], &stmt)) return JIM_ERROR;
	...
	if (Jim_GetInt(interp, args[2], &idx)) return JIM_ERROR;
	int bytes;
	char *value = (char *)Jim_GetByteArray(interp, args[3], &bytes);
...
__device__ static int test_bind_text16(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Vdbe *stmt;
	if (getStmtPointer(interp, oStmt, &stmt)) return JIM_ERROR;
	int idx;
	if (Jim_GetInt(interp, oN, &idx)) return JIM_ERROR;
	char *value = (char *)Jim_GetByteArray(interp, oString, nullptr);
	int bytes;
...
__device__ static int test_bind_blob(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 5 && argc != 6)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " STMT N DATA BYTES", nullptr);
		return JIM_ERROR;
	}
...
__device__ static int test_bind_parameter_count(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Vdbe *stmt;
	if (getStmtPointer(interp, args[1], &stmt)) return JIM_ERROR;
	Jim_SetObjResult(interp, Vdbe::Bind_ParameterCount(stmt));
	return JIM_OK;
}
...
__device__ static int test_bind_parameter_name(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Jim_SetObjResult(interp, Vdbe::Bind_ParameterName(stmt,i));
	return JIM_OK;
}
...
__device__ static int test_bind_parameter_index(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Vdbe *stmt;
	if (getStmtPointer(interp, args[1], &stmt)) return JIM_ERROR;
	Jim_SetObjResult(interp, Vdbe::Bind_ParameterIndex(stmt, args[2]));
	return JIM_OK;
}
...
__device__ static int test_clear_bindings(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Vdbe *stmt;
	if (getStmtPointer(interp, args[1], &stmt)) return JIM_ERROR;
	Jim_SetObjResult(interp, stmt->ClearBindings());
	return JIM_OK;
}
...
__device__ static int test_sleep(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Jim_SetObjResult(interp, Main::Sleep(ms));
	return JIM_OK;
}
...
__device__ static int test_ex_errcode(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Jim_AppendResult(interp, (char *)t1ErrorName(rc), 0);
	return JIM_OK;
}
...
__device__ static int test_errmsg16(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Jim_SetObjResult(interp, array_t<const void>(err, bytes));
#endif
	return JIM_OK;
}
...
__device__ static int test_prepare(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 5 && argc != 4)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " DB sql bytes ?tailvar?", nullptr);
		return JIM_ERROR;
	}
	Context *ctx;
	if (getDbPointer(interp, args[1], &ctx)) return JIM_ERROR;
	...
	if (tail && argc >= 5)
	{
		...
		Jim_SetVar2(interp, (char *)args[4], 0, Jim_NewStringObj(tail, bytes), 0);
	}
	char buf[50];
	if (rc != RC_OK)
	{
...
__device__ static int test_prepare_v2(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	if (tail && argc >= 5)
	{
		if (bytes >= 0)
			bytes = bytes - (int)(tail-sql);
		Jim_SetVar2(interp, (char *)args[4], nullptr, Jim_NewStringObj(tail, bytes), 0);
	}
	char buf[50];
	if (rc != RC_OK)
	{
...
__device__ static int test_prepare16(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	const void *sql = Jim_GetByteArray(interp, args[2], &sqlLength);
	...
	if (argc >= 5)
	{
		...
		Jim_SetVar2(interp, (char *)args[4], nullptr, Jim_NewByteArray(tail, bytes), 0);
	}
	char buf[50];
	if (stmt)
		if (sqlite3TestMakePointerStr(interp, buf, stmt)) return JIM_ERROR;
...
__device__ static int test_open(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " filename options-list", nullptr);
		return JIM_ERROR;
	}
...
__device__ static int test_open_v2(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	int flagLength;
	const char **flags;
	int rc = Jim_ListObjGetElements(interp, (char *)args[2], &flagLength, &flags);
	if (rc != JIM_OK) return rc;
	VSystem::OPEN flags2 = (VSystem::OPEN)0;
	for (int i = 0; i < flagLength; i++)
	{
		int flagId;
		rc = Jim_GetIndex(interp, flags[i], (const void **)_flags, sizeof(_flags[0]), "flag", 0, &flagId);
		if (rc != JIM_OK) return rc;
...
__device__ static int test_open16(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	const void *filename = Jim_GetByteArray(interp, args[1], nullptr);
	Context *ctx;
...
__device__ static int test_complete16(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	char *buf = (char *)Jim_GetByteArray(interp, args[1], nullptr);
	Jim_SetObjResult(interp, Parse::Complete16(buf));
#endif
	return JIM_OK;
}
...
__device__ static int test_step(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Jim_SetResult(interp, (char *)t1ErrorName(rc), nullptr);
	return JIM_OK;
}
...
__device__ static int test_sql(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Vdbe *stmt;
	if (getStmtPointer(interp, args[1], &stmt)) return JIM_ERROR;
	Jim_SetResult(interp, (char *)Vdbe::Sql(stmt), JIM_VOLATILE);
	return JIM_OK;
}
...
__device__ static int test_column_count(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Vdbe *stmt;
	if (getStmtPointer(interp, args[1], &stmt)) return JIM_ERROR;
	Jim_SetObjResult(interp, Vdbe::Column_Count(stmt));
	return JIM_OK;
}
...
__device__ static int test_column_type(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	switch (tp)
	{
	case TYPE_INTEGER: Jim_SetResult(interp, "INTEGER", JIM_STATIC); break;
	case TYPE_NULL: Jim_SetResult(interp, "NULL", JIM_STATIC); break;
	case TYPE_FLOAT: Jim_SetResult(interp, "FLOAT", JIM_STATIC); break;
	case TYPE_TEXT: Jim_SetResult(interp, "TEXT", JIM_STATIC); break;
	case TYPE_BLOB: Jim_SetResult(interp, "BLOB", JIM_STATIC); break;
	default: _assert(false);
	}
	return JIM_OK;
}
...
__device__ static int test_column_blob(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Jim_SetObjResult(interp, array_t<const void>(blob, len));
	return JIM_OK;
}
...
__device__ static int test_data_count(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Jim_SetObjResult(interp, Vdbe::Data_Count(stmt));
	return JIM_OK;
}
...
__device__ static int test_stmt_utf8(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	if (ret)
		Jim_SetResult(interp, (char *)ret, nullptr);
	return JIM_OK;
}
...
__device__ static int test_stmt_utf16(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	if (name16)
	{
		...
		Jim_SetObjResult(interp, array_t<const void>(name16, n+2));
	}
#endif
	return JIM_OK;
}
...
__device__ static int test_stmt_int(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	Jim_SetObjResult(interp, func(stmt, col));
	return JIM_OK;
}
...
__device__ static int sqlite_set_magic(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 3)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " DB MAGIC", nullptr);
		return JIM_ERROR;
	}
	Context *ctx;
	if (getDbPointer(interp, args[1], &ctx)) return JIM_ERROR;
...
__device__ static int tcl_variable_type(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	char *var = Jim_GetVar2(interp, (char *)args[1], nullptr, JIM_LEAVE_ERR_MSG);
	if (!var) return JIM_ERROR;
	if (var->TypePtr)
		Jim_SetObjResult(interp, var->TypePtr->Name);
	return JIM_OK;
}
...
__device__ static int test_db_readonly(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	if (getDbPointer(interp, args[1], &ctx)) return JIM_ERROR;
	const char *dbName = args[2];
	Jim_SetObjResult(interp, Main::CtxReadonly(ctx, dbName));
	return JIM_OK;
}
...
__device__ static int test_soft_heap_limit(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	int64 N = -1;
	if (argc == 2)
		if (Jim_GetWideInt(interp, args[1], &N)) return JIM_ERROR;
...
__device__ static int test_pager_refcounts(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	for (int i = 0; i < ctx->DBs.length; i++)
	{
		...
		Jim_ListObjAppendElement(nullptr, result, (int)v);
	}
	Jim_SetObjResult(interp, result);
	return JIM_OK;
}
...
{
	char *testObj = Jim_NewWideInt(1000000*(int64)1234567890);
	bool working = !_strcmp((char *)testObj, "1234567890000000");
	Jim_DecrRefCount(testObj);
	Jim_SetObjResult(interp, working);
	return JIM_OK;
}
...
__device__ static int file_control_lasterrno_test(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	if (rc)
	{ 
		Jim_SetObjResult(interp, (int)rc); 
		return JIM_ERROR; 
	}
	if (argId != 0)
	{
...
__device__ static int file_control_chunksize_test(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	if (getDbPointer(interp, args[1], &ctx) || Jim_GetInt(interp, args[3], &size)) return JIM_ERROR;
	...
	if (rc)
	{
		Jim_SetResult(interp, (char *)sqlite3TestErrorName(rc), JIM_STATIC);
		return JIM_ERROR;
	}
	return JIM_OK;
}
...
__device__ static int file_control_sizehint_test(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	if (getDbPointer(interp, args[1], &ctx) || Jim_GetWideInt(interp, args[3], &size)) return JIM_ERROR;
	const char *dbName = args[2]; // Db name ("main", "temp" etc.)
	if (dbName[0] == '\0') dbName = nullptr;
...
__device__ static int file_control_lockproxy_test(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 3)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " DB PWD", nullptr);
		return JIM_ERROR;
	}
	Context *ctx;
	if (getDbPointer(interp, args[1], &ctx)) return JIM_ERROR;
...
# define ENABLE_LOCKING_STYLE 0
...
#if ENABLE_LOCKING_STYLE && defined(__APPLE__)
	{
		...
		{
			Jim_AppendResult(interp, "PWD too big", (void*)0);
			return JIM_ERROR;
		}
...
__device__ static int file_control_win32_av_retry(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 4)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " DB NRETRY DELAY", nullptr);
		return JIM_ERROR;
	}
	Context *ctx;
	if (getDbPointer(interp, args[1], &ctx)) return JIM_ERROR;
...
__device__ static int file_control_persist_wal(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 3)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " DB FLAG", nullptr);
		return JIM_ERROR;
	}
	Context *ctx;
	if (getDbPointer(interp, args[1], &ctx)) return JIM_ERROR;
...
__device__ static int file_control_vfsname(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 2 && argc != 3)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " DB ?AUXDB?", nullptr);
		return JIM_ERROR;
	}
	Context *ctx;
	if (getDbPointer(interp, args[1], &ctx)) return JIM_ERROR;
...
__device__ static int vfs_list(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	for (VSystem *vfs = VSystem::FindVfs(nullptr); vfs; vfs = vfs->Next)
		Jim_ListObjAppendElement(interp, ret, vfs->Name);
...
__device__ static int test_limit(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	if (argc != 4)
	{
		Jim_AppendResult(interp, "wrong # args: should be \"", args[0], " DB ID VALUE", nullptr);
		return JIM_ERROR;
	}
	Context *ctx;
	if (getDbPointer(interp, args[1], &ctx)) return JIM_ERROR;
...
__device__ static int test_pcache_stats(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	char *ret = Jim_NewObj();
	Jim_ListObjAppendElement(interp, ret, "current");
	Jim_ListObjAppendElement(interp, ret, (int)current);
	Jim_ListObjAppendElement(interp, ret, "max");
	Jim_ListObjAppendElement(interp, ret, (int)max);
	Jim_ListObjAppendElement(interp, ret, "min");
	Jim_ListObjAppendElement(interp, ret, (int)min);
	Jim_ListObjAppendElement(interp, ret, "recyclable");
	Jim_ListObjAppendElement(interp, ret, (int)recyclables);
	Jim_SetObjResult(interp, ret);
	return JIM_OK;
}
...
__device__ static int test_wal_checkpoint_v2(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	if (getDbPointer(interp, args[1], &ctx) || Jim_GetIndex(interp, args[2], (const char **)_modes, "mode", 0, (int *)&mode)) return JIM_ERROR;
	...
	Jim_Obj *ret = Jim_NewObj();
	Jim_ListObjAppendElement(interp, ret, (rc == RC_BUSY ? 1 : 0));
	Jim_ListObjAppendElement(interp, ret, logs);
	Jim_ListObjAppendElement(interp, ret, ckpts);
	Jim_SetObjResult(interp, ret);
	return JIM_OK;
}
...
{
	char *new_ = Jim_DuplicateObj(_logcallback.Obj);
	Jim_IncrRefCount(new_);
	Jim_ListObjAppendElement(0, new_, sqlite3TestErrorName(err));
	Jim_ListObjAppendElement(0, new_, msg);
	Jim_Eval(_logcallback.Interp, new_, 0, nullptr);
	Jim_DecrRefCount(new_);
}
__device__ static int test_sqlite3_log(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	if (_logcallback.Obj)
	{
		Jim_DecrRefCount(_logcallback.Obj);
...
__device__ static int runAsObjProc(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	if (!cmdInfo.objProc)
	{
		Jim_AppendResult(interp, "command has no objProc: ", args[1], nullptr);
		return JIM_ERROR;
	}
...
__device__ static int test_test_control(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[])
{
	...
	int verbId;
	int rc = Jim_GetIndex(interp, args[1], (const void **)_verbs, sizeof(_verbs[0]), "VERB", 0, &verbId);
	if (rc != JIM_OK) return rc;
...
__device__ extern int sqlite3BtreeSharedCacheReport(ClientData clientData, Jim_Interp *interp, int argc, Jim_Obj *const args[]);
