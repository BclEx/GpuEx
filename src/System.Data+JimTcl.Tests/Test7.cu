...
int sqlite3_client_open(const char *, Context **);
int sqlite3_client_prepare(Context *, const char *, int, Vdbe **, const char **);
int sqlite3_client_step(Vdbe *);
int sqlite3_client_reset(Vdbe *);
int sqlite3_client_finalize(Vdbe *);
int sqlite3_client_close(Context *);
...
struct Thread
{
	// The first group of fields are writable by the supervisor thread and read-only to the client threads
	...
	// The next group of fields are writable by the client threads but read-only to the superviser thread.
	volatile int completed;  // Number of operations completed
	Context *db;             // Open database
	Vdbe *pStmt;     // Pending operation
	char *zErr;              // operation error
	char *zStaticErr;        // Static error message
	RC rc;                  // operation return code
	int argc;                // number of columns in result
	const char *argv[100];   // result columns
	const char *colv[100];   // result column names
};

// There can be as many as 26 threads running at once.  Each is named by a capital letter: A, B, C, ..., Y, Z.
#define N_THREAD 26
...
*/
static int parse_client_id(Jim_Interp *interp, const char *zArg){
	if( zArg==0 || zArg[0]==0 || zArg[1]!=0 || !isupper((unsigned char)zArg[0]) ){
		Jim_AppendResult(interp, "thread ID must be an upper case letter", 0);
		return -1;
	}
	return zArg[0] - 'A';
}
...
*/
int Sqlitetest7_Init(Jim_Interp *interp){
	static struct {
		char *zName;
		Jim_CmdProc *xProc;
	} aCmd[] = {
		{ "client_create",     (Jim_CmdProc *)tcl_client_create     },
		{ "client_wait",       (Jim_CmdProc *)tcl_client_wait       },
		{ "client_halt",       (Jim_CmdProc *)tcl_client_halt       },
		{ "client_argc",       (Jim_CmdProc *)tcl_client_argc       },
		{ "client_argv",       (Jim_CmdProc *)tcl_client_argv       },
		{ "client_colname",    (Jim_CmdProc *)tcl_client_colname    },
		{ "client_result",     (Jim_CmdProc *)tcl_client_result     },
		{ "client_error",      (Jim_CmdProc *)tcl_client_error      },
		{ "client_compile",    (Jim_CmdProc *)tcl_client_compile    },
		{ "client_step",       (Jim_CmdProc *)tcl_client_step       },
		{ "client_reset",      (Jim_CmdProc *)tcl_client_reset      },
		{ "client_finalize",   (Jim_CmdProc *)tcl_client_finalize   },
		{ "client_swap",       (Jim_CmdProc *)tcl_client_swap       },
	};
	int i;
...
#else
__device__ int Sqlitetest7_Init(Jim_Interp *interp)
{
	return JIM_OK;
}
#endif
