...
__device__ static void set_options(Tcl_Interp *interp)
{
	...
	Tcl_SetVar2(interp, "sqlite_options", "malloc_usable_size", "1", TCL_GLOBAL_ONLY);
#else
	Tcl_SetVar2(interp, "sqlite_options", "malloc_usable_size", "0", TCL_GLOBAL_ONLY);
#endif
	...
	Tcl_SetVar2(interp, "sqlite_options", "long_double", (sizeof(LONGDOUBLE_TYPE) > sizeof(double) ? "1" : "0"), TCL_GLOBAL_ONLY);
...
#define LINKVAR(x) { static const int cv_ ## x = SQLITE_ ## x; Tcl_LinkVar(interp, "SQLITE_" #x, (char *)&(cv_ ## x), TCL_LINK_INT|TCL_LINK_READ_ONLY); }
	LINKVAR(MAX_LENGTH);
	LINKVAR(MAX_COLUMN);
	...
	LINKVAR(MAX_DEFAULT_PAGE_SIZE);
	{
		...
		Tcl_LinkVar(interp, "TEMP_STORE", (char *)&(cv_TEMP_STORE), TCL_LINK_INT|TCL_LINK_READ_ONLY);
	}

#ifdef _MSC_VER
	{
		static const int cv__MSC_VER = 1;
		Tcl_LinkVar(interp, "_MSC_VER", (char *)&(cv__MSC_VER), TCL_LINK_INT|TCL_LINK_READ_ONLY);
	}
#endif
#ifdef __GNUC__
	{
		static const int cv___GNUC__ = 1;
		Tcl_LinkVar(interp, "__GNUC__", (char *)&(cv___GNUC__), TCL_LINK_INT|TCL_LINK_READ_ONLY);
	}
#endif
}
