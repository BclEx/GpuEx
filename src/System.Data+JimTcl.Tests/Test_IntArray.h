// This is the C-language interface definition for the "intarray" or integer array virtual table for SQLite.
//
// The intarray virtual table is designed to facilitate using an array of integers as the right-hand side of an IN operator.  So
// instead of doing a prepared statement like this:
//
//     SELECT * FROM table WHERE x IN (?,?,?,...,?);
//
// And then binding indivdual integers to each of ? slots, a C-language application can create an intarray object (named "ex1" in the following
// example), prepare a statement like this:
//
//     SELECT * FROM table WHERE x IN ex1;
//
// Then bind an ordinary C/C++ array of integer values to the ex1 object to run the statement.
//
// USAGE:
//
...
//
// Each call to sqlite3_intarray_create() generates a new virtual table module and a singleton of that virtual table module in the TEMP
// database.  Both the module and the virtual table instance use the name given by the second parameter.  The virtual tables can then be
...
//
// A single intarray object can be rebound multiple times.  But do not attempt to change the bindings of an intarray while it is in the middle
// of a query.
//
// The array that holds the integers is automatically freed by the function in the fourth parameter to sqlite3_intarray_bind() when the array is no
// longer needed.  The application must not change the intarray values while an intarray is in the middle of a query.
//
// The intarray object is automatically destroyed when its corresponding virtual table is dropped.  Since the virtual tables are created in the
// TEMP database, they are automatically dropped when the database connection closes so the application does not normally need to take any special
