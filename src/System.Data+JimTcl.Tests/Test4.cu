...
struct Thread
{
	// The first group of fields are writable by the master and read-only to the thread.
	...
	// The next group of fields are writable by the thread but read-only to the master.
	int Completed;			// Number of operations completed
	Context *Ctx;           // Open database
	Vdbe *Stmt;			// Pending operation
	char *Err;				// operation error
	char *StaticErr;		// Static error message
