...
#pragma region BLOB
#ifdef xyx //OMIT_INCRBLOB

// Close all incrblob channels opened using database connection pDb.
// This is called when shutting down the database connection.
static void CloseIncrblobChannels(TestCtx *tctx)
{
...
static int IncrblobClose(ClientData instanceData, void *interp)
{
	IncrblobChannel *p = (IncrblobChannel *)instanceData;
	int rc = sqlite3_blob_close(p->Blob);
	Context *ctx = p->TCtx->Ctx;
	...
	if (p->Prev)
		p->Prev->Next = p->Next;
	if (p->TCtx->Incrblob == p)
		p->TCtx->Incrblob = p->Next;
...
__device__ static bool SafeToUseEvalObjv(Tcl_Interp *interp, Tcl_Obj *cmd)
{
	...
	int n;
	const char *z = cmd->GetStringFromObj(&n);
	while (n-- > 0)
	{
		int c = *(z++);
...
__device__ static void FlushStmtCache(TestCtx *tctx)
{
...
__device__ static void DbDeleteCmd(void *db)
{
	...
	if (tctx->NullText)
		_free(tctx->NullText);
	if (tctx->UpdateHook)
		tctx->UpdateHook->DecrRefCount();
	if (tctx->RollbackHook)
		tctx->RollbackHook->DecrRefCount();
	if (tctx->WalHook)
		tctx->WalHook->DecrRefCount();
	if (tctx->CollateNeeded)
		tctx->CollateNeeded->DecrRefCount();
	_free(tctx);
}
...
__device__ static void DbProfileHandler(void *cd, const char *sql, uint64 tm)
{
	TestCtx *tctx = (TestCtx *)cd;
	char b[100];
	__snprintf(b, sizeof(b)-1, "%lld", tm);
	//Tcl_DString str;
	//Tcl_DStringInit(&str);
...
{
	TestCtx *tctx = (TestCtx *)clientData;
	_assert(tctx->RollbackHook);
	if (tctx->Interp->EvalObjEx(tctx->RollbackHook, 0) != RC_OK)
		tctx->Interp->BackgroundError();
}
...
__device__ static int DbWalHandler(void *clientData,  Context *ctx, const char *dbName,  int entrys)
{
	...
	Tcl_Obj *p = tctx->WalHook->DuplicateObj();
	p->IncrRefCount();
	p->ListObjAppendElement(interp, Tcl_Obj::NewStringObj(dbName, -1));
	p->ListObjAppendElement(interp, Tcl_Obj::NewIntObj(entrys));
	if (interp->EvalObjEx(p, 0) != RC_OK || interp->GetObjResult()->GetIntFromObj(interp, &rc) != RC_OK)
		tctx->Interp->BackgroundError();
	p->DecrRefCount();
	return rc;
}
...
__device__ static void SetTestUnlockNotifyVars(Tcl_Interp *interp, int argId, int argsLength)
{
	...
	//interp->SetVar("sqlite_unlock_notify_arg", b, TCL_GLOBAL_ONLY);
	__snprintf(b, sizeof(b), "%d", argsLength);
	//interp->SetVar("sqlite_unlock_notify_argcount", b, TCL_GLOBAL_ONLY);
}
#else
...
__device__ static void DbUnlockNotify(void **args, int argsLength)
{
	for (int i = 0; i < argsLength; i++)
	{
		...
		TestCtx *tctx = (TestCtx *)args[i];
		SetTestUnlockNotifyVars(tctx->Interp, i, argsLength);
		_assert(tctx->UnlockNotify);
		tctx->Interp->EvalObjEx(tctx->UnlockNotify, flags);
		tctx->UnlockNotify->DecrRefCount();
...
__device__ static void DbUpdateHandler(void *p, TK op, const char *dbName, const char *tableName,  int64 rowid)
{
	...
	tctx->Interp->EvalObjEx(cmd, TCL_EVAL_DIRECT);
	cmd->DecrRefCount();
}
...
{
	TestCtx *tctx = (TestCtx *)p;
	Tcl_Obj *script = tctx->CollateNeeded->DuplicateObj();
	script->IncrRefCount();
	script->ListObjAppendElement(nullptr, Tcl_Obj::NewStringObj(name, -1));
	tctx->Interp->EvalObjEx(script, 0);
	script->DecrRefCount();
}
...
__device__ static int TclSqlCollate(void *p1, int aLength, const void *a, int bLength, const void *b)
{
	...
	cmd->IncrRefCount();
	cmd->ListObjAppendElement(p->Interp, Tcl_Obj::NewStringObj((const char *)a, aLength));
...
__device__ static void TclSqlFunc(FuncContext *fctx, int argc, Mem **argv)
{
	...
	else
	{
		...
		Tcl_Obj **args;
		int argsLength;
		if (p->Script->ListObjGetElements(p->Interp, &argsLength, &args))
		{
			Vdbe::Result_Error(fctx, p->Interp->GetStringResult(), -1); 
			return;
		}     
		...
		for (int i = 0; i < argc; i++)
		{
			...
			case TYPE_INTEGER: {
				...
				val = (v >= -2147483647 && v <= 2147483647 ? Tcl_Obj::NewIntObj((int)v) : Tcl_Obj::NewWideIntObj(v));
				break; }
			case TYPE_FLOAT: {
				...
				break; }
			case TYPE_NULL: {
				val = Tcl_Obj::NewStringObj(p->Ctx->NullText, -1);
				break; }
			default: {
				int bytes = Vdbe::Value_Bytes(in);
		...
		if (!p->UseEvalObjv)
			cmd->GetString();
		rc = p->Interp->EvalObjEx(cmd, TCL_EVAL_DIRECT);
		cmd->DecrRefCount();
	}
	...
	{
		Tcl_Obj *var = p->Interp->GetObjResult();
		int n;
		uint8 *data;
...
__device__ static ARC AuthCallback(void *pArg, int code, const char *zArg1, const char *zArg2, const char *zArg3, const char *zArg4)
{
	TestCtx *tctx = (TestCtx *)pArg;
	...
	//Tcl_DStringFree(&str);
	ARC rc = ARC_OK;
	const char *reply = (rc == RC_OK ? tctx->Interp->GetStringResult() : "ARC_DENY");
...
__device__ static int DbTransPostCmd(ClientData data[], Tcl_Interp *interp, int result)
{
	...
	if (Main::Exec(ctx, end, nullptr, nullptr, nullptr))
	{
		...
		if (rc != RC_ERROR)
		{
			interp->AppendResult(Tcl_Obj::NewStringObj(Main::ErrMsg(ctx), -1), 0);
			rc = RC_ERROR;
		}
...
__device__ static RC DbPrepareAndBind(TestCtx *tctx, char const *sql, char const **out, SqlPreparedStmt **preStmt)
{
	Context *ctx = tctx->Ctx;
	Tcl_Interp *interp = tctx->Interp;
	*preStmt = nullptr;
	...
	if (!p)
	{
		...
		{
			interp->SetObjResult(Tcl_Obj::NewStringObj(Main::ErrMsg(ctx), -1));
			return RC_ERROR;
		}
		if (!stmt)
		{
	...
	for (int i = 1; i <= vars; i++)
	{
		...
		if (varName && (varName[0] == '$' || varName[0] == ':' || varName[0] == '@'))
		{
			...
			if (var)
			{
				...
				{
					var->GetIntFromObj(interp, &n);
					Vdbe::Bind_Int(stmt, i, n);
				}
				else if (c == 'd' && !_strcmp(typeName, "double"))
				{
					double r;
					var->GetDoubleFromObj(interp, &r);
					Vdbe::Bind_Double(stmt, i, r);
				}
				else if ((c == 'w' && !_strcmp(typeName, "wideInt")) || (c == 'i' && !_strcmp(typeName, "int")))
				{
					int64 v;
					var->GetWideIntFromObj(interp, &v);
...
__device__ static void DbReleaseStmt(TestCtx *tctx, SqlPreparedStmt *preStmt, bool discard)
{
	// Free the bound string and blob parameters
	for (int i = 0; i < preStmt->Parms.length; i++)
		preStmt->Parms[i]->DecrRefCount();
...
//
//     set ${pArray}(*) {a b c}
__device__ static void DbEvalInit(DbEvalContext *p, TestCtx *tctx, Tcl_Obj *sql, Tcl_Obj *array)
{
...
__device__ static int DbMain(void *cd, Tcl_Interp *interp, array_t<Tcl_Obj *> objv)
{
	...
	char *errMsg = nullptr;
	TestCtx *p = (TestCtx *)_alloc(sizeof(*p));
	if (!p)
	{
