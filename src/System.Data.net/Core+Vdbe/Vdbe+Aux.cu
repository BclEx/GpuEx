...
__device__ static Vdbe::VdbeOp *OpIterNext(VdbeOpIter *p)
{
...
#ifndef NDEBUG
__device__ void Vdbe::Comment_(const char *format, _va_list &args)
{
...
__device__ void Vdbe::NoopComment_(const char *format, _va_list &args)
{
	AddOp0(OP_Noop);
	_assert(Ops.length > 0 || !Ops.data);
	_assert(!Ops.data || !Ops[Ops.length-1].Comment || Ctx->MallocFailed);
