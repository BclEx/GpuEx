...
// The following declarations either map ckalloc and ckfree to malloc and free, or they map them to procedures with all sorts of debugging hooks defined in tclCkalloc.c.
#ifdef TCL_MEM_DEBUG
EXTERN __device__ char *Tcl_DbCkalloc (unsigned int size, char *file, int line);
EXTERN __device__ int Tcl_DbCkfree(char *ptr, char *file, int line);
EXTERN __device__ char *Tcl_DbCkrealloc(char *ptr, unsigned int size, char *file, int line);
...
#define Tcl_FreeResult(interp) \
	...
	if ((interp)->freeProc == (Tcl_FreeProc *)free) { ckfree((interp)->result); } \
...
EXTERN __device__ void Tcl_AppendResult(Tcl_Interp *interp, ...) ATTRIBUTE_SENTINEL;
...
EXTERN __device__ void Tcl_SetErrorCode(Tcl_Interp *interp, ...) ATTRIBUTE_SENTINEL;
...
EXTERN __device__ int Tcl_VarEval(Tcl_Interp *interp, ...) ATTRIBUTE_SENTINEL;
