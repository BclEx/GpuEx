...
__device__ static void InitHistory(register Interp *iPtr)
{
	...
	iPtr->events = (HistoryEvent *)
		_allocFast((unsigned) (iPtr->numEvents * sizeof(HistoryEvent)));
...
infoCmd:
		...
		*/
		...
		events = (HistoryEvent *)
			_allocFast((unsigned) (count * sizeof(HistoryEvent)));
		if (count < iPtr->numEvents) {
...
__device__ static void RevCommand(register Interp *iPtr, char *string)
{
	...
	revPtr = (HistoryRev *) _allocFast(sizeof(HistoryRev));
...
__device__ static HistoryEvent *GetEvent(register Interp *iPtr, char *string)
{
	...
	*/
	...
	for (index = iPtr->curEvent - 1; ; index--) {
		...
		if ((_strncmp(eventPtr->command, string, (size_t) length) == 0)
...
__device__ static int SubsAndEval(register Interp *iPtr, char *cmd, char *old, char *new_)
{
	...
	*/
	...
	oldLength = _strlen(old);
	newLength = _strlen(new_);
	src = cmd;
	count = 0;
	while (1) {
	...
	length = _strlen(cmd) + count*(newLength - oldLength);

	/*
	* Generate a substituted command.
	*/

	newCmd = (char *) _allocFast((unsigned) (length + 1));
	dst = newCmd;
	while (1) {
	...
	result = Tcl_Eval((Tcl_Interp *) iPtr, newCmd, 0, 0);
	_freeFast(newCmd);
	return result;
}

...
__device__ static char *GetWords(register Interp *iPtr, char *command, char *words)
{
	...
	*/
	...
	result = (char *) _allocFast((unsigned) (strlen(command) + 1));
	dst = result;
	for (next = command; _isspace(*next); next++) {
	...
	for (index = 0; *next != 0; index++) {
		...
		if (*end != 0) {
			...
			for (next = end; _isspace(*next); next++) {
