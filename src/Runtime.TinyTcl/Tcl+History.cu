...
*----------------------------------------------------------------------
*/
__device__ static void InitHistory(register Interp *iPtr)
{
	if (iPtr->numEvents != 0) {
		return;
	}
...
*----------------------------------------------------------------------
*/
__device__ int Tcl_HistoryCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
	register Interp *iPtr = (Interp *)interp;
	...
	if (c == 'a' && !_strncmp(argv[1], "add", length)) {
		if (argc != 3 && argc != 4) {
			Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0], " add event ?exec?\"", (char *)NULL);
			return TCL_ERROR;
		}
		if (argc == 4) {
			if (_strncmp(argv[3], "exec", _strlen(argv[3]))) {
				Tcl_AppendResult(interp, "bad argument \"", argv[3], "\": should be \"exec\"", (char *)NULL);
				return TCL_ERROR;
			}
		...
		if (argc != 3 && argc != 4) {
			Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0], " change newValue ?event?\"", (char *)NULL);
			return TCL_ERROR;
		}
		if (argc == 3) {
		...
		if (argc > 3) {
			Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0], " event ?event?\"", (char *)NULL);
			return TCL_ERROR;
		}
		...
		if (argc != 2 && argc != 3) {
			Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0], " info ?count?\"", (char *)NULL);
			return TCL_ERROR;
		}
infoCmd:
		int count;
		...
		if (argc != 3) {
			Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0], " keep number\"", (char *)NULL);
			return TCL_ERROR;
		}
		int count;
		if (Tcl_GetInt(interp, argv[2], &count) != TCL_OK) {
		...
		if (count <= 0 || count > 1000) {
			Tcl_AppendResult(interp, "illegal keep count \"", argv[2], "\"", (char *)NULL);
			return TCL_ERROR;
		}
		...
		if (argc != 2) {
			Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0], " nextid\"", (char *)NULL);
			return TCL_ERROR;
		}
		...
		if (argc > 3) {
			Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0], " redo ?event?\"", (char *)NULL);
			return TCL_ERROR;
		}
		eventPtr = GetEvent(iPtr, (argc == 2 ? "-1" : argv[2]));
		if (eventPtr == NULL) {
		...
		if (argc > 5 || argc < 4) {
			Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0], " substitute old new ?event?\"", (char *)NULL);
			return TCL_ERROR;
		}
		...
		if (argc != 3 && argc != 4) {
			Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0], " words num-num/pat ?event?\"", (char *)NULL);
			return TCL_ERROR;
		}
		...
		char *words = GetWords(iPtr, eventPtr->command, argv[2]);
		if (words == NULL) {
			return TCL_ERROR;
		}
	...
	Tcl_AppendResult(interp, "bad option \"", argv[1], "\": must be add, change, event, info, keep, nextid, ", "redo, substitute, or words", (char *)NULL);
	return TCL_ERROR;
}

/*
...
*----------------------------------------------------------------------
*/
__device__ static void InsertRev(Interp *iPtr, register HistoryRev *revPtr)
{
...
*----------------------------------------------------------------------
*/
__device__ static void RevCommand(register Interp *iPtr, char *string)
{
	...
	revPtr->newSize = _strlen(string);
	revPtr->newBytes = (char *)_allocFast((unsigned)(revPtr->newSize+1));
	_strcpy(revPtr->newBytes, string);
...
*----------------------------------------------------------------------
*/
__device__ static void RevResult(register Interp *iPtr, char *string)
{
	if (iPtr->evalFirst == NULL || iPtr->revDisables > 0) {
		return;
	}
	...
	revPtr->newSize = _strlen(revPtr->newBytes);
	InsertRev(iPtr, revPtr);
}

/*
...
*----------------------------------------------------------------------
*/
__device__ static void DoRevs(register Interp *iPtr)
{
	if (iPtr->revPtr == NULL) {
		return;
	}
	...
	register HistoryEvent *eventPtr = &iPtr->events[iPtr->curEvent];
	unsigned int size = _strlen(eventPtr->command) + 1;
	...
	for (revPtr = iPtr->revPtr; revPtr != NULL;) {
		...
		if (count > 0) {
			_strncpy(p, eventPtr->command + bytesSeen, (size_t)count);
			p += count;
		}
		_strncpy(p, revPtr->newBytes, (size_t)revPtr->newSize);
		p += revPtr->newSize;
	...
	_strcpy(p, eventPtr->command + bytesSeen);
...
*----------------------------------------------------------------------
*/
__device__ static HistoryEvent *GetEvent(register Interp *iPtr, char *string)
{
	...
	for (index = iPtr->curEvent - 1; ; index--) {
		...
		if (!_strncmp(eventPtr->command, string, (size_t)length) || Tcl_StringMatch(eventPtr->command, string)) {
			return eventPtr;
		}
	}
...
*----------------------------------------------------------------------
*/
__device__ static int SubsAndEval(register Interp *iPtr, char *cmd, char *old, char *new_)
{
	...
	int oldLength = _strlen(old);
	int newLength = _strlen(new_);
	char *src = cmd;
	int count = 0;
	while (true) {
		src = (char *)_strstr(src, old);
		if (src == NULL) {
			break;
		}
		src += oldLength;
	...
	int length = _strlen(cmd) + count*(newLength - oldLength);

	// Generate a substituted command.
	char *newCmd = (char *)_allocFast((unsigned)(length + 1));
	char *dst = newCmd;
	while (true) {
		src = (char *)_strstr(cmd, old);
		if (src == NULL) {
			_strcpy(dst, cmd);
			break;
		}
		_strncpy(dst, cmd, (size_t)(src-cmd));
		dst += src-cmd;
		_strcpy(dst, new_);
		dst += newLength;
		cmd = src + oldLength;
...
*----------------------------------------------------------------------
*/
__device__ static char *GetWords(register Interp *iPtr, char *command, char *words)
{
	...
	if (*words == '$') {
		...
		first = -1;
	} else if (_isdigit(*words)) {
		first = _strtoul(words, &start, 0);
		if (*start == 0) {
			last = first;
		} else if (*start == '-') {
			start++;
			if (*start == '$') {
				start++;
			} else if (_isdigit(*start)) {
				last = _strtoul(start, &start, 0);
			} else {
				goto error;
			}
			if (*start != 0) {
	...
	register char *next;
	char *result = (char *)_allocFast((unsigned)(_strlen(command) + 1));
	char *dst = result;
	for (next = command; _isspace(*next); next++) { } // Empty loop body:  just find start of first word.
	for (index = 0; *next != 0; index++) {
		start = next;
		char *end = TclWordEnd(next, 0);
		if (*end != 0) {
			end++;
			for (next = end; _isspace(*next); next++) { } // Empty loop body:  just find start of next word.
		}
		...
		_strncpy(dst, start, (size_t)(end-start));
		dst += end-start;
	}
	*dst = 0;
