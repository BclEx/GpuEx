...
__device__ int Tcl_IfCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
	...
	while (1) {
		...
		*/
		...
		i++;
		if ((i < argc) && (_strcmp(argv[i], "then") == 0)) {
			i++;
		}
		if (i >= argc) {
...
__device__ int Tcl_InfoCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
	...
	if ((c == 'a') && (_strncmp(argv[1], "args", length)) == 0) {
		if (argc != 3) {
...
__device__ int Tcl_LrangeCmd(ClientData notUsed, Tcl_Interp *interp, int argc, char **argv)
{
	...
	if ((*argv[3] == 'e') && (_strncmp(argv[3], "end", _strlen(argv[3])) == 0)) {
		last = 1000000;
	} else {
...
__device__ static char *sort_command = 0;
...
__device__ static int CommandSortCompareProc(const VOID *first, const VOID *second)
{
	...
	return _atoi(sort_interp->result);
}
...
__device__ int Tcl_LsortCmd(ClientData notUsed, Tcl_Interp *interp, int argc, char **argv)
{
	...
	while (argc > 2) {
		argc--;
		argv++;
		if (_strcmp(argv[0], "-integer") == 0) {
			compare = (compare_function_type *)IntegerSortCompareProc;
			break;
		}
		if (_strcmp(argv[0], "-command") == 0) {
