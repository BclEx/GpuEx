...
__device__ int Tcl_GlobalCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
	...
	for (argc--, argv++; argc > 0; argc--, argv++) {
		hPtr = Tcl_CreateHashEntry(&iPtr->globalTable, *argv, &new_);
		if (new_) {
			gVarPtr = NewVar(0);
		...
		hPtr2 = Tcl_CreateHashEntry(&iPtr->varFramePtr->varTable, *argv, &new_);
		if (!new_) {
			Var *varPtr;
...
upvarSyntax:
	...
	*/
	...
	while (argc > 0) {
		hPtr = Tcl_CreateHashEntry(upVarTablePtr, argv[0], &new_);
		if (new_) {
			upVarPtr = NewVar(0);
