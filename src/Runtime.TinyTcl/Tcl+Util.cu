	...
	while (1) {
		...
		case '}':
			if (openBraces == 1) {
				...
				for (p2 = p; (*p2 != 0) && (!_isspace(*p2)) && (p2 < p+20);
					p2++) {
						/* null body */
				}
				Tcl_ResetResult(interp);
				_sprintf(interp->result,
...
__device__ char *Tcl_Concat(int argc, char **argv)
{
	...
	for (p = result, i = 0; i < argc; i++) {
		...
		*/
		...
		for (length = _strlen(element);
			(length > 0) && (_isspace(element[length-1]));
			length--) {
				/* Null loop body. */
		}
		if (length == 0) {
		...
		(void) _strncpy(p, element, length);
		p += length;
		*p = ' ';
		p++;
	}
...
__device__ int TclGetListIndex(Tcl_Interp *interp, char *string, int *indexPtr)
{
	...
	} else if (_strncmp(string, "end", _strlen(string)) == 0) {
		*indexPtr = 1<<30;
	} else {
...
__device__ regex_t *TclCompileRegexp(Tcl_Interp *interp, char *string, int nocase)
{
	...
	for (i = 0; i < iPtr->num_regexps; i++) {
		if ((length == iPtr->regexps[i].length)
			...
			&& (_strcmp(string, iPtr->regexps[i].pattern) == 0)) {
				/*
