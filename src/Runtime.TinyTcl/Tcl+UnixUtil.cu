...
int Tcl_EvalFile(Tcl_Interp *interp, char *fileName)
{
	...
	if (fileId < 0) {
		Tcl_AppendResult(interp, "couldn't read file \"", fileName, "\": ", Tcl_UnixError(interp), (char *)NULL);
		goto error;
	}
	struct stat statBuf;
	if (fstat(fileId, &statBuf) == -1) {
		Tcl_AppendResult(interp, "couldn't stat file \"", fileName, "\": ", Tcl_UnixError(interp), (char *)NULL);
		close(fileId);
		goto error;
	}
	char *cmdBuffer = (char *)_allocFast((unsigned)statBuf.st_size+1);
	if (read(fileId, cmdBuffer, (int) statBuf.st_size) != statBuf.st_size) {
		Tcl_AppendResult(interp, "error in reading file \"", fileName, "\": ", Tcl_UnixError(interp), (char *)NULL);
		close(fileId);
		_freeFast(cmdBuffer);
		goto error;
	}
	if (close(fileId) != 0) {
		Tcl_AppendResult(interp, "error closing file \"", fileName, "\": ", Tcl_UnixError(interp), (char *)NULL);
		_freeFast(cmdBuffer);
		goto error;
	}
...
#define TMP_STDIN_NAME "/tmp/tcl.in.XXXXXX"
...
#endif
			if (inputId < 0) {
				Tcl_AppendResult(interp, "couldn't create input file for command: ", Tcl_UnixError(interp), (char *)NULL);
				goto error;
			}
			int length = strlen(input);
			if (write(inputId, input, length) != length) {
				Tcl_AppendResult(interp, "couldn't write file input for command: ", Tcl_UnixError(interp), (char *)NULL);
				goto error;
			}
			if (lseek(inputId, 0L, 0) == -1 || unlink(inName) == -1) {
				Tcl_AppendResult(interp, "couldn't reset or remove input file for command: ", Tcl_UnixError(interp), (char *)NULL);
				goto error;
			}
		} else if (inputFile == 2) {
			...
			if (inputId < 0) {
				Tcl_AppendResult(interp, "couldn't read file \"", input, "\": ", Tcl_UnixError(interp), (char *)NULL);
				goto error;
			}
		}
	} else if (inPipePtr != NULL) {
		if (pipe(pipeIds) != 0) {
			Tcl_AppendResult(interp, "couldn't create input pipe for command: ", Tcl_UnixError(interp), (char *)NULL);
			goto error;
		}
		inputId = pipeIds[0];
		*inPipePtr = pipeIds[1];
	...
	if (output != NULL) {
		if (outputFile == 2) {
			...
			if (lastOutputId < 0) {
				Tcl_AppendResult(interp, "couldn't write file \"", output, "\": ", Tcl_UnixError(interp), (char *)NULL);
				goto error;
			}
		}
		...
		if (pipe(pipeIds) != 0) {
			Tcl_AppendResult(interp, "couldn't create output pipe: ", Tcl_UnixError(interp), (char *)NULL);
			goto error;
		}
	...
	if (error != NULL) {
		if (errorFile == 2) {
			...
			if (errorId < 0) {
				Tcl_AppendResult(interp, "couldn't write file \"", error, "\": ", Tcl_UnixError(interp), (char *)NULL);
				goto error;
			}
		}
...
#define TMP_STDERR_NAME "/tmp/tcl.err.XXXXXX"
		...
		if (errorId < 0) {
errFileError:
			Tcl_AppendResult(interp, "couldn't create error file for command: ", Tcl_UnixError(interp), (char *)NULL);
			goto error;
		}
		*errFilePtr = open(errName, O_RDONLY, 0);
		...
		if (unlink(errName) == -1) {
			Tcl_AppendResult(interp, "couldn't remove error file for command: ", Tcl_UnixError(interp), (char *)NULL);
			goto error;
		}
	}
	...
	for (int firstArg = 0; firstArg < argc; numPids++, firstArg = lastArg+1) {
		...
		if (lastArg == argc) {
			...
			if (pipe(pipeIds) != 0) {
				Tcl_AppendResult(interp, "couldn't create pipe: ", Tcl_UnixError(interp), (char *)NULL);
				goto error;
			}
			outputId = pipeIds[1];
		}
		...
		int pid = Tcl_Fork();
		if (pid == -1) {
			Tcl_AppendResult(interp, "couldn't fork child process: ", Tcl_UnixError(interp), (char *)NULL);
			goto error;
		}
		if (pid == 0) {
			char errSpace[200];
...
#endif
		iPtr->filePtrArray = (OpenFile **)_allocFast((unsigned)((iPtr->numFiles)*sizeof(OpenFile *)));
...
*----------------------------------------------------------------------
*/
int TclGetOpenFile(Tcl_Interp *interp, char *string, OpenFile **filePtrPtr)
{
