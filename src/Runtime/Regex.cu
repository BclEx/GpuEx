...
__device__ static int reg(regex_t *preg, int paren, int *flagp);
__device__ static int regpiece(regex_t *preg, int *flagp);
__device__ static int regbranch(regex_t *preg, int *flagp);
__device__ static int regatom(regex_t *preg, int *flagp);
__device__ static int regnode(regex_t *preg, int op);
__device__ static int regnext(regex_t *preg, int p);
__device__ static void regc(regex_t *preg, int b);
__device__ static int reginsert(regex_t *preg, int op, int size, int opnd);
__device__ static void regtail(regex_t *preg, int p, int val);
__device__ static void regoptail(regex_t *preg, int p, int val);
__device__ static int regopsize(regex_t *preg, int p );
__device__ static int reg_range_find(const int *string, int c);
__device__ static const char *str_find(const char *string, int c, int nocase);
__device__ static int prefix_cmp(const int *prog, int proglen, const char *string, int nocase);

#ifdef _DEBUG
__device__ static int regnarrate = 0;
__device__ static void regdump(regex_t *preg);
__device__ static const char *regprop(int op);
#endif

// Returns the length of the null-terminated integer sequence.
__device__ static __inline int _strlenint(const int *seq)
{
	int n = 0;
	while (*seq++) { n++; }
	return n;
...
__device__ static int regpiece(regex_t *preg, int *flagp)
{
	...
	{
		char *end;
		min = _strtoul(preg->regparse + 1, &end, 10);
		...
		{
			preg->regparse = end;
			max = _strtoul(preg->regparse + 1, &end, 10);
		...
		preg->regparse = _strchr(preg->regparse, '}');
	}
	else
	{
		min = (op == '+');
...
__device__ static void reg_addrange(regex_t *preg, int lower, int upper)
{
	if (lower > upper)
...
__device__ static void reg_addrange_str(regex_t *preg, const char *str)
{
	while (*str)
	{
...
__device__ static int reg_utf8_tounicode_case(const char *s, int *uc, int upper)
{
...
__device__ static int hexdigitval(int c)
{
...
__device__ static int parse_hex(const char *s, int n, int *uc)
{
	int val = 0;
	int k;
	for (k = 0; k < n; k++)
...
__device__ static int reg_decode_escape(const char *s, int *ch)
{
	int n;
	const char *s0 = s;
	*ch = *s++;
...
de_fault:
	default: {
		...
		while (*preg->regparse && _strchr(META, *preg->regparse) == NULL)
		{
...
__device__ static void reg_grow(regex_t *preg, int n)
{
	if (preg->p + n >= preg->proglen)
	{
		preg->proglen = (preg->p + n) * 2;
		preg->program = (int *)_realloc(preg->program, preg->proglen * sizeof(int));
	}
}
...
__device__ static int reginsert(regex_t *preg, int op, int size, int opnd)
{
	...
	_memmove(preg->program + opnd + size, preg->program + opnd, sizeof(int) * (preg->p - opnd));
	// Zero out the new space
	_memset(preg->program + opnd, 0, sizeof(int) * size);
...
// regexec and friends
// Forwards.
__device__ static int regtry(regex_t *preg, const char *string);
__device__ static int regmatch(regex_t *preg, int prog);
__device__ static int regrepeat(regex_t *preg, int p, int max);
...
__device__ static int reg_range_find(const int *range, int c)
{
	while (*range)
	{
...
__device__ static int reg_iseol(regex_t *preg, int ch)
{
	return (preg->cflags & REG_NEWLINE ? ch == '\0' || ch == '\n' : ch == '\0');
}

__device__ static int regmatchsimplerepeat(regex_t *preg, int scan, int matchmin)
{
...
__device__ static int regmatchrepeat(regex_t *preg, int scan, int matchmin)
{
...
__device__ static int regmatch(regex_t *preg, int prog)
{
	...
	while (scan != 0)
	{
		...
		case OP_WORDA:
			...
			if (preg->reginput > preg->regbol && (_isalnum(preg->reginput[-1]) || preg->reginput[-1] == '_'))
				return 0;
			break;
		case OP_WORDZ:
			// Can't match at BOL
			if (preg->reginput > preg->regbol) {
				...
				if (reg_iseol(preg, c) || !_isalnum(c) || c != '_')
				{
					c = preg->reginput[-1];
...
__device__ static void regdump(regex_t *preg)
{
	...
	{
		_printf("%02x ", (unsigned char)preg->program[i]);
		if (i % 16 == 0)
			_printf("\n");
	}
	_printf("\n");
	...
	{
		op = OP(preg, s);
		_printf("%3d: %s", s, regprop(op)); // Where, what
	...
	if (op == OP_END)
	{
		...
		if (preg->regmust != 0)
		{
			...
			for (int i = 0; i < preg->regmlen; i++)
				_putchar(preg->program[preg->regmust + i]);
			_putchar('\n');
		}
	}
	_printf("\n");
...
__device__ size_t regerror(int errcode, const regex_t *preg, char *errbuf, size_t errbuf_size)
{
	...
	return __snprintf(errbuf, errbuf_size, "%s", err);
}
