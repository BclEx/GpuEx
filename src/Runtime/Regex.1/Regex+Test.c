...
int main(void) {
  ...
  ASSERT(slre_match("x", "abcd", 4, NULL, 0, 0) == REG_NO_MATCH);
  ASSERT(slre_match(".", "abcd", 4, NULL, 0, 0) == 1);
  ASSERT(slre_match("^.*\\\\.*$", "c:\\Tools", 8, NULL, 0, REG_IGNORE_CASE)
    == 8);
  ASSERT(slre_match("\\", "a", 1, NULL, 0, 0) == REG_INVALID_METACHARACTER);
  ASSERT(slre_match("\\x", "a", 1, NULL, 0, 0) == REG_INVALID_METACHARACTER);
  ASSERT(slre_match("\\x1", "a", 1, NULL, 0, 0) == REG_INVALID_METACHARACTER);
  ASSERT(slre_match("\\x20", " ", 1, NULL, 0, 0) == 1);

  ASSERT(slre_match("^.+$", "", 0, NULL, 0, 0) == REG_NO_MATCH);
  ASSERT(slre_match("^(.+)$", "", 0, NULL, 0, 0) == REG_NO_MATCH);
  ASSERT(slre_match("^([\\+-]?)([\\d]+)$", "+", 1,
                    caps, 10, REG_IGNORE_CASE) == REG_NO_MATCH);
  ASSERT(slre_match("^([\\+-]?)([\\d]+)$", "+27", 3,
                    caps, 10, REG_IGNORE_CASE) == 3);
  ASSERT(caps[0].len == 1);
  ASSERT(caps[0].ptr[0] == '+');
  ASSERT(caps[1].len == 2);
  ...
  ASSERT(slre_match("[abc]", "1C2", 3, NULL, 0, 0) == REG_NO_MATCH);
  ASSERT(slre_match("[abc]", "1C2", 3, NULL, 0, REG_IGNORE_CASE) == 2);
  ASSERT(slre_match("[.2]", "1C2", 3, NULL, 0, 0) == 1);
  ASSERT(slre_match("[\\S]+", "ab cd", 5, NULL, 0, 0) == 2);
  ASSERT(slre_match("[\\S]+\\s+[tyc]*", "ab cd", 5, NULL, 0, 0) == 4);
  ASSERT(slre_match("[\\d]", "ab cd", 5, NULL, 0, 0) == REG_NO_MATCH);
  ...
  ASSERT(slre_match("FO", "foo", 3, NULL, 0, 0) == REG_NO_MATCH);
  ASSERT(slre_match("FO", "foo", 3, NULL, 0, REG_IGNORE_CASE) == 2);
  ASSERT(slre_match("(?m)FO", "foo", 3, NULL, 0, 0) ==
    REG_UNEXPECTED_QUANTIFIER);
  ...
  ASSERT(slre_match("p+", "fooklmn", 7, NULL, 0, 0) == REG_NO_MATCH);
  ASSERT(slre_match("ok", "fooklmn", 7, NULL, 0, 0) == 4);
  ASSERT(slre_match("lmno", "fooklmn", 7, NULL, 0, 0) == REG_NO_MATCH);
  ASSERT(slre_match("mn.", "fooklmn", 7, NULL, 0, 0) == REG_NO_MATCH);
  ASSERT(slre_match("o", "fooklmn", 7, NULL, 0, 0) == 2);
  ASSERT(slre_match("^o", "fooklmn", 7, NULL, 0, 0) == REG_NO_MATCH);
  ASSERT(slre_match("^", "fooklmn", 7, NULL, 0, 0) == 0);
  ASSERT(slre_match("n$", "fooklmn", 7, NULL, 0, 0) == 7);
  ASSERT(slre_match("n$k", "fooklmn", 7, NULL, 0, 0) == REG_NO_MATCH);
  ASSERT(slre_match("l$", "fooklmn", 7, NULL, 0, 0) == REG_NO_MATCH);
  ...
  ASSERT(slre_match("\\_", "abc", 3, NULL, 0, 0) == REG_INVALID_METACHARACTER);
  ASSERT(slre_match("+", "fooklmn", 7, NULL, 0, 0) == REG_UNEXPECTED_QUANTIFIER);
  ASSERT(slre_match("()+", "fooklmn", 7, NULL, 0, 0) == REG_NO_MATCH);
  ASSERT(slre_match("\\x", "12", 2, NULL, 0, 0) == REG_INVALID_METACHARACTER);
  ASSERT(slre_match("\\xhi", "12", 2, NULL, 0, 0) == REG_INVALID_METACHARACTER);
  ASSERT(slre_match("\\x20", "_ J", 3, NULL, 0, 0) == 2);
  ASSERT(slre_match("\\x4A", "_ J", 3, NULL, 0, 0) == 3);
  ASSERT(slre_match("\\d+", "abc123def", 9, NULL, 0, 0) == 6);

  /* Balancing brackets */
  ASSERT(slre_match("(x))", "fooklmn", 7, NULL, 0, 0) == REG_UNBALANCED_BRACKETS);
  ASSERT(slre_match("(", "fooklmn", 7, NULL, 0, 0) == REG_UNBALANCED_BRACKETS);

  ASSERT(slre_match("klz?mn", "fooklmn", 7, NULL, 0, 0) == 7);
  ASSERT(slre_match("fa?b", "fooklmn", 7, NULL, 0, 0) == REG_NO_MATCH);
  ...
  ASSERT(slre_match("a|b|c", "d", 1, NULL, 0, 0) == REG_NO_MATCH);

  /* Optional match at the end of the string */
  ASSERT(slre_match("^.*c.?$", "abc", 3, NULL, 0, 0) == 3);
  ASSERT(slre_match("^.*C.?$", "abc", 3, NULL, 0, REG_IGNORE_CASE) == 3);
