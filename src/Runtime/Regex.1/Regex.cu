...
static int match_op(const unsigned char *re, const unsigned char *s,
  ...
  switch (*re) {
    case '\\':
      /* Metacharacters */
      switch (re[1]) {
        case 'S': FAIL_IF(isspace(*s), REG_NO_MATCH); result++; break;
        case 's': FAIL_IF(!isspace(*s), REG_NO_MATCH); result++; break;
        case 'd': FAIL_IF(!isdigit(*s), REG_NO_MATCH); result++; break;
        case 'b': FAIL_IF(*s != '\b', REG_NO_MATCH); result++; break;
        case 'f': FAIL_IF(*s != '\f', REG_NO_MATCH); result++; break;
        case 'n': FAIL_IF(*s != '\n', REG_NO_MATCH); result++; break;
        case 'r': FAIL_IF(*s != '\r', REG_NO_MATCH); result++; break;
        case 't': FAIL_IF(*s != '\t', REG_NO_MATCH); result++; break;
        case 'v': FAIL_IF(*s != '\v', REG_NO_MATCH); result++; break;

        case 'x':
          /* Match byte, \xHH where HH is hexadecimal byte representaion */
          FAIL_IF(hextoi(re + 2) != *s, REG_NO_MATCH);
          result++;
          break;

        default:
          /* Valid metacharacter check is done in bar() */
          FAIL_IF(re[1] != s[0], REG_NO_MATCH);
          result++;
          break;
      }
      break;

    case '|': FAIL_IF(1, REG_INTERNAL_ERROR); break;
    case '$': FAIL_IF(1, REG_NO_MATCH); break;
    case '.': result++; break;

    default:
      if (info->flags & REG_IGNORE_CASE) {
        FAIL_IF(tolower(*re) != tolower(*s), REG_NO_MATCH);
      } else {
        FAIL_IF(*re != *s, REG_NO_MATCH);
      }
      result++;
      break;
  }
...
static int match_set(const char *re, int re_len, const char *s,
  ...
  while (len <= re_len && re[len] != ']' && result <= 0) {
    ...
    if (re[len] != '-' && re[len + 1] == '-' && re[len + 2] != ']' &&
        re[len + 2] != '\0') {
      result = info->flags &&  REG_IGNORE_CASE ?
        *s >= re[len] && *s <= re[len + 2] :
...
static int bar(const char *re, int re_len, const char *s, int s_len,
  ...
  for (i = j = 0; i < re_len && j <= s_len; i += step) {
    ...
    FAIL_IF(is_quantifier(&re[i]), REG_UNEXPECTED_QUANTIFIER);
    FAIL_IF(step <= 0, REG_INVALID_CHARACTER_SET);
    ...
    if (i + step < re_len && is_quantifier(re + i + step)) {
      ...
      if (re[i + step] == '?') {
        ...
        FAIL_IF(re[i + step] == '+' && nj == j, REG_NO_MATCH);

        /* If while loop body above was not executed for the * quantifier,  */
        /* make sure the rest of the regex matches                          */
        FAIL_IF(nj == j && ni < re_len && n2 < 0, REG_NO_MATCH);
    ...
    if (re[i] == '[') {
      ...
      FAIL_IF(n <= 0, REG_NO_MATCH);
      j += n;
    } else if (re[i] == '(') {
      n = REG_NO_MATCH;
      bi++;
      FAIL_IF(bi >= info->num_brackets, REG_INTERNAL_ERROR);
      DBG(("CAPTURING [%.*s] [%.*s] [%s]\n",
      ...
      FAIL_IF(j != 0, REG_NO_MATCH);
    } else if (re[i] == '$') {
      FAIL_IF(j != s_len, REG_NO_MATCH);
    } else {
      FAIL_IF(j >= s_len, REG_NO_MATCH);
...
static int foo(const char *re, int re_len, const char *s, int s_len,
  ...
  for (i = 0; i < re_len; i += step) {
    ...
    if (re[i] == '|') {
      ...
      FAIL_IF(i >= re_len - 1, REG_INVALID_METACHARACTER);
      if (re[i + 1] == 'x') {
        /* Hex digit specification must follow */
        FAIL_IF(re[i + 1] == 'x' && i >= re_len - 3,
                REG_INVALID_METACHARACTER);
        FAIL_IF(re[i + 1] ==  'x' && !(isxdigit(re[i + 2]) &&
                isxdigit(re[i + 3])), REG_INVALID_METACHARACTER);
      } else {
      ...
      depth--;
      FAIL_IF(depth < 0, REG_UNBALANCED_BRACKETS);
      FAIL_IF(i > 0 && re[i - 1] == '(', REG_NO_MATCH);
    }
  }
