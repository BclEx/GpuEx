...
__device__ static int clock_cmd_format(Jim_Interp *interp, int argc, Jim_Obj *const *argv)
{
...
__device__ static int clock_cmd_seconds(Jim_Interp *interp, int argc, Jim_Obj *const *argv)
{
	Jim_SetResultInt(interp, _time(NULL));
	return JIM_OK;
}

__device__ static int clock_cmd_micros(Jim_Interp *interp, int argc, Jim_Obj *const *argv)
{
	struct timeval tv;
...
__device__ static int clock_cmd_millis(Jim_Interp *interp, int argc, Jim_Obj *const *argv)
{
	struct timeval tv;
	_gettimeofday(&tv, NULL);
