...
#define JIM_MODFLAG_FULLARGV 0x0002		/* Subcmd proc gets called with full argv */
	...
	/*
	* Returns JIM_OK if OK, JIM_ERROR (etc.) on error, break, continue, etc.
	...
	* NULL will be returned and an appropriate error will be set if the subcommand or arguments are invalid.
	*
	* Typical usage is:
	*  {
	...
	*/
	...
	/*
	* Parses the args against the given command table and executes the subcommand if found or sets an appropriate error if the subcommand or arguments is invalid.
	*
	...
	* by Jim_ParseSubCmd()
	*
	* If ct is NULL, returns JIM_ERROR, leaving any message.
	* Otherwise invokes ct->function
	*
	* If ct->function returns -1, sets an error message and returns JIM_ERROR.
