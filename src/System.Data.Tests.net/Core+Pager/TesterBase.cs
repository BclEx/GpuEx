...
{
    protected void puts(string v)
    {
    }

    #region tester.tcl

    static string tcl_platform_platform = "windows";
    protected Dictionary<string, object> G;
    protected bool _do_not_use_codec;

    protected void sqlite3(object db, string[] args)
    {
        if (args.Length >= 1 && args[0][0] != '-')
        {
            // This command is opening a new database connection.
            if (G.ContainsKey("perm:sqlite3_args"))
            {
            ...
            var res = 0; //[uplevel 1 sqlite_orig $args]
            if (G.ContainsKey("perm:presql"))
            {
                //[lindex $args 0] eval G["perm:presql")
            }
            if (G.ContainsKey("perm:dbconfig"))
            {
    ...
    protected int GetFileRetries()
    {
        if (!G.ContainsKey("file-retries"))
        {
            // NOTE: Return the default number of retries for [file] operations.  A value of zero or less here means "disabled".
            return (tcl_platform_platform == "windows" ? 50 : 0);
        }
        return (int)G["file-retries"];
    }

    protected int GetFileRetryDelay()
    {
        if (!G.ContainsKey("file-retry-delay"))
        {
    ...
    protected void copy_file(string from, string to) { do_copy_file(false, from, to); }
    protected void forcecopy(string from, string to) { do_copy_file(true, from, to); }
    protected void do_copy_file(bool force, string from, string to)
    {
        var retrys = GetFileRetries(); // Maximum number of retries.
        var delays = GetFileRetryDelay(); // Delay in ms before retrying.
    ...
    protected bool is_relative_file(string file)
    {
        Debugger.Break();
    ...
    protected bool test_pwd(string[] args)
    {
        //  if {[llength $args] > 0} {
    ...
    protected void delete_file(params string[] args) { do_delete_file(false, args); }
    protected void forcedelete(params string[] args) { do_delete_file(true, args); }
    protected void do_delete_file(bool force, string[] args)
    {
        var retrys = GetFileRetries();     // Maximum number of retries.
        var delays = GetFileRetryDelay();  // Delay in ms before retrying.
    ...
    protected void ExecPresql(object handle, object args)
    {
        Debugger.Break();
    ...
    protected void do_not_use_codec()
    {
    ...
    protected void execsql(string sql, Context db = null)
    {
        // puts "SQL = $sql"
    ...
    #region lock_common.tcl

    public class MultiClientTesterBase : TesterBase
    {
