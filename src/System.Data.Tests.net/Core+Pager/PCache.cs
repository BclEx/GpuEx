...
// Only works with a mode-2 pcache where all pcaches share a single set of pages.
// #ifcapable {!memorymanage && threadsafe}
using Xunit;
public class PCache : TesterBase
{
    // The pcache module limits the number of pages available to purgeable caches to the sum of the 'cache_size' values for the set of open
    // caches. This block of tests, pcache-1.*, test that the library behaves corrctly when it is forced to exceed this limit.
    [Fact]
    public void T1()
    {
        if (false)
        ...
        // At this point connection db2 has a read lock on the database file and a single pinned page in its cache. Connection [db] is holding 10 dirty 
        ...
        // Rolling back the transaction held by db2 at this point releases a pinned page. Because the number of allocated pages is greater than the 
        // configured maximum, this page should be freed immediately instead of recycled.
        do_test("pcache-1.8", () =>
        {
